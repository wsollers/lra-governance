...
def _topic_dirs(parent: Path) -> set[str]:
...
def _validate_chapter(volume_root: Path, chapter: Path, findings: list[Finding], schema: dict) -> None:
    ...
    _validate_notes_shape(volume_root, chapter, findings, schema)
    _validate_proofs_shape(volume_root, chapter, findings, schema)


def _validate_notes_shape(volume_root: Path, chapter: Path, findings: list[Finding], schema: dict) -> None:
...
def _validate_proofs_shape(volume_root: Path, chapter: Path, findings: list[Finding], schema: dict) -> None:
    ...
    notes_topics = _topic_dirs(chapter / "notes")
    proof_topics = _topic_dirs(proofs_root)
    ...
    for topic in sorted((notes_topics - proof_topics) - note_only_topics):
        ...
        if _note_topic_requires_proofs(notes_topic, schema):
...
def _note_topic_requires_proofs(topic_root: Path, schema: dict) -> bool:
