...
- `validate_note_blocks.py` - target- or path-scoped validator for generated note prose
  block discipline.
...
Run from `lra-governance` against the note files, note directory, volume,
chapter, or section generated by the current task. Prefer the narrowest
meaningful target:

```powershell
python tools\governance\validate_note_blocks.py --root F:\repos\lra-volume-ii --chapter whole-numbers --section extending-addition
```

This validator still accepts explicit paths for ad hoc checks, but target flags
