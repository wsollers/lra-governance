...
SPEC = importlib.util.spec_from_file_location(
    "validate_topic_semantic_audit", MODULE_PATH
)
...
class TopicSemanticAuditValidatorTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        canonical_schema_dir = (
            Path(__file__).resolve().parents[2] / "constitution" / "schema"
        )
        for name in (
            ...
            schema_dir.joinpath(name).write_text(
                canonical_schema_dir.joinpath(name).read_text(encoding="utf-8"),
                encoding="utf-8",
            )
        self.topic = (
            self.repo
    ...
    def audit_record(
        ...
        *,
        revert_commit: str | None = "c" * 40,
        restored: bool = True,
    ) -> dict:
        checks = {
            name: {"result": "pass", "notes": None}
            for name in (
                "binder_scope",
                "language_level",
        ...
        return {
            ...
            "artifact": {
                ...
                "kind": "definition",
                "source_file": (
                    "volume-iii/book-analysis-i/bounding/notes/"
                    "bounds-extremals/notes-bounds-extremals.tex"
                ),
            ...
            "review_package": {
                "result": "pass",
                "review_commit": "a" * 40,
                "semantic_artifact_command": "validate_semantic_artifact.py",
                "exit_code": 0,
                "findings": [],
            },
                ...
                "changed_files": [
                    {
                        ...
                        "pre_apply_blob_sha": "1" * 40,
                        "tested_blob_sha": "2" * 40,
                    }
                ],
            },
                ...
                "checks": [
                    {
                        ...
                        "command": "validate_semantic_artifact.py",
                        "result": "pass",
                        "exit_code": 0,
                        "started_at": None,
                        "completed_at": None,
            ...
            "logic_validation": {
                ...
                "reviewer": {
                    ...
                    "prompt": (
                        ...
                        "validate-semantic-artifact-logic.md"
                    ),
                    "run_id": None,
                },
                "checks": checks,
    ...
    def write_package(
        ...
        *,
        ...
        rel = (
            Path(
                "volume-iii/book-analysis-i/bounding/notes/bounds-extremals"
            )
            / slug
        )
        ...
        **overrides,
        ...
        artifact = {
            ...
            "audit_validation_file": (
                f"{review_directory}/audit-validation.yaml"
            ),
            "status": "validated",
            "review_commit": "a" * 40,
    ...
    def test_rejects_temporary_commit_without_revert(self):
        review_directory = self.write_package(
            "def-supremum", revert_commit=None
        )
        ...
        self.assertTrue(
            any(
                "matching revert" in message
                or "revert_commit" in message
    ...
    def test_rejects_incorrect_summary_counts(self):
        ...
        self.assertTrue(
            any(
                "summary.validated_count" in message
                for message in result.errors
            )
        )
    ...
    def test_allows_queued_inventory_without_review_folders_non_strict(self):
        review_directory = (
            "volume-iii/book-analysis-i/bounding/notes/"
            "bounds-extremals/def-supremum"
        )
        data = self.manifest(
            review_directory,
            status="queued",
            review_commit=None,
            temporary_apply_commit=None,
            revert_commit=None,
            deterministic_result="not_run",
            logic_result="not_run",
