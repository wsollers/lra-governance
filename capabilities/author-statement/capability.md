...
- `python <governance-root>/tools/governance/validate_semantic_logic.py --repos-root <repos-root> --volume <i-viii> --book <book> --chapter <chapter> --section <section> --label <statement-label> --output <logic-validation.yaml>` for scoped create-then-validate; use `--no-create-missing` only for fast inspection.
