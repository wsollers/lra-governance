...
Scope:
- Volume content only. Edit only this volume's `volume-iv/` content unless the
...
- Chapters route through `volume-iv/index.tex` and the owning book root. Chapter
...
Overlay-specific verifier args: Verifiers pass `--no-require-box`.
