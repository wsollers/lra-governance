...
| Prepare Lean <-> TeX verification lookup | `docs/governance/source-indexing-policy.md`, `docs/governance/repo-overlays/lra-lean.md`, `docs/architecture/lra-lean-architecture.md`, `docs/governance/repo-overlays/lra-volume.md` | `docs/governance/source-indexing-policy.yaml`; target TeX volume repo(s); `[external:lra-lean] LRA/` | `tools/governance/index_internal_objects.py`, downstream crosswalk or verifier scripts | `docs/architecture/local-semantic-logic-verifier.md`, `docs/architecture/semantic-artifact-record.md` when comparing formal statement ASTs | ignored internal object index, curated reviewed crosswalk candidates when requested, optional PDF page-map enrichment | index generation succeeds; YAML/JSON parses; do not use PDFs as primary object-discovery source |
